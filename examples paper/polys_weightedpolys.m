...
indfun = @(x) (x >= -1).*(x <= 1);
integrator = @(f) integral(@(x) arrayevaluation(f,x)/2, -1, 1, 'RelTol', 1e-2);
maxchrist = 1e6;
...
% comparison of u vs k_n^epsilon close to the singularity
...
% exportgraphics(f1, "weightedpolysA1.pdf", 'ContentType', 'vector');
...
text(0,10^(1.8),'iteration 8','Interpreter','latex','FontSize',20,'HorizontalAlignment','center');
% exportgraphics(f2, "weightedpolysA2.pdf", 'ContentType', 'vector');
